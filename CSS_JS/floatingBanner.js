...
window.addEventListener('scroll', () => {
  ...
  // ▼ フェードイン / アウト
  if (y > SHOW_SCROLL_Y) {banner.classList.add('is-visible');}
  else {banner.classList.remove('is-visible');}

  // ▼ ページ下部に近づいたらバナーを少し上に避ける
  const bottomDistance = docHeight - (y + winHeight);

  if (bottomDistance < FOOTER_MARGIN) {banner.style.bottom = `${FOOTER_MARGIN - bottomDistance + 20}px`;}
  else {banner.style.bottom = "20px";}});
